...
<p class="text-justify">Après examen de votre dossier, nous avons le regret de porter à votre connaissance qu’une suite défavorable a été réservée à votre demande et vous demandons de bien vouloir vous présenter à l’agence CNAC de {{ $employeur->wilaya->des_fr }} à partir du {{ Carbon\Carbon::parse($demande->date_decision)->format("d-m-Y") }} pour retirer votre <b><i>DECISION DE REJET DE LA SUBVENTION MENSUELLE.</i></b> pour motif <i><b> « {{ !is_null($demande->cod_rejet) ? ($demande->cod_rejet == 5 ? $demande->description_rejet : App\Models\Employeur::motifRejet($demande->cod_rejet)) : ""}} ».</i></b></p>

<br />
<p class="text-justify"> <b> Si vous désirez contester la decision de rejet, vous avez la possibilité de déposer un recours adressé à Monsieur le Président de la Commission de Recours territorialement compétente dans les huit jours à compter de la date de réception de votre decision.</b></p>
<br>
<p class="text-right">
